...
export default function MeteoLcdConsole({ 
  ...
  const outdoorTemp = fusedTemp !== undefined ? fusedTemp : Math.round((hourly.temperature_2m?.[currentIdx] ?? current.temperature_2m ?? 20) * 10) / 10;
  const outdoorHumidity = fusedHumidity !== undefined ? fusedHumidity : Math.round(hourly.relative_humidity_2m?.[currentIdx] ?? current.relative_humidity_2m ?? 65);
  const feelsLike = fusedApparentTemp !== undefined ? fusedApparentTemp : Math.round((hourly.apparent_temperature?.[currentIdx] ?? current.apparent_temperature ?? outdoorTemp) * 10) / 10;
  const pressure = fusedPressure !== undefined && fusedPressure > 800 ? fusedPressure : Math.round(hourly.pressure_msl?.[currentIdx] ?? current.pressure_msl ?? 1029);
  const windSpeed = fusedWindSpeed !== undefined ? Math.round(fusedWindSpeed) : Math.round(hourly.wind_speed_10m?.[currentIdx] ?? current.wind_speed_10m ?? 12);
  const windGust = fusedWindGusts !== undefined ? Math.round(fusedWindGusts) : Math.round(hourly.wind_gusts_10m?.[currentIdx] ?? current.wind_gusts_10m ?? (windSpeed * 1.4));
  const windDirDeg = fusedWindDirection !== undefined ? Math.round(fusedWindDirection) : Math.round(hourly.wind_direction_10m?.[currentIdx] ?? current.wind_direction_10m ?? 180);
  const windDirLabel = getWindDirection(windDirDeg);
  const uvVal = fusedUvIndex !== undefined ? fusedUvIndex : (hourly.uv_index?.[currentIdx] ?? current.uv_index ?? 3.5);
              ...
              <div className={`p-3 rounded-xl border ${themeStyle.lcdGrid} ${themeStyle.cardBg}`}>
                ...
                <div className="grid grid-cols-2 gap-2 my-1">
                  <div>
                    ...
                    <div className="flex items-baseline space-x-1">
                      <span className={`text-2xl font-black ${theme === "classic" ? "text-purple-300" : themeStyle.textMain}`}>{pressure}</span>
                  ...
                  <div>
                    ...
                    <div className="flex items-baseline space-x-1">
                      <span className={`text-2xl font-black ${theme === "classic" ? "text-amber-300" : themeStyle.textMain}`}>{feelsLike}°</span>
