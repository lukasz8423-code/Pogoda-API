...
export default React.memo(function WeatherAdviceCards({ data }: WeatherAdviceCardsProps) {
  ...
  const generateAdvice = (): AdviceItem[] => {
    ...
    const currentPop = hourly && hourly.precipitation_probability ? hourly.precipitation_probability[0] : 0;
    ...
    const rawMaxUv = daily.uv_index_max ? daily.uv_index_max[0] : (current.uv_index ?? 0);
