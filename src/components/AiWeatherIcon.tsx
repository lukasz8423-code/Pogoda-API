...
export default function AiWeatherIcon({
  ...
  const getWeatherDetails = () => {
    ...
    if ((code >= 71 && code <= 77) || (code >= 85 && code <= 86)) {
      return {
        icon: code === 71 ? Snowflake : CloudSnow,
        color: "text-sky-200",
    ...
    if ((code >= 63 && code <= 67) || code === 82 || precip > 1.5) {
      return {
        icon: CloudRain,
        color: "text-blue-300",
    ...
    if ((code >= 51 && code <= 62) || (code >= 80 && code <= 81) || precip > 0) {
      return {
        icon: CloudDrizzle,
        color: "text-cyan-300",
    ...
    if (code === 2 || (cloudCover >= 30 && cloudCover < 70)) {
      return {
    ...
    if (code === 1 || (cloudCover > 5 && cloudCover < 30)) {
      return {
        icon: isDay ? CloudSun : CloudMoon,
        color: isDay ? "text-amber-200" : "text-indigo-200",
