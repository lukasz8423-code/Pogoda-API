...
/**
 * Calculates apparent (feels-like) temperature compliant with IMGW-PIB / JAGTI / Osczevski / NOAA standard:
 * 1. Wind Chill (wzór Jagti/Osczevski-Bluestein) stosowany tylko przy niskich temperaturach (T <= 10°C) i wietrze >= 4.8 km/h.
 * 2. Humidex / Heat Index stosowany tylko przy wyższych temperaturach (T >= 24°C) i parności.
 * 3. Strefa komfortu (10°C < T < 24°C): odczuwalna jest bardzo zbliżona do temperatury rzeczywistej, bez sztucznego zaniżania.
 */
export function calculateApparentTemperature(
  ...
  if (temp <= 10 && ws >= 4.8) {
    const vPow = Math.pow(ws, 0.16);
    const wc = 13.12 + (0.6215 * temp) - (11.37 * vPow) + (0.3965 * temp * vPow);
    return Number(wc.toFixed(1));
  }

  // 2. GORĄCO: Humidex / Heat Index przy wyższych temperaturach (T >= 24°C)
  if (temp >= 24) {
    const es = 6.105 * Math.exp((17.27 * temp) / (237.7 + temp));
    const e = es * (rh / 100);
    const humidex = temp + (5 / 9) * (e - 10);
    if (humidex > temp) {
      const windRelief = Math.min(2.0, 0.3 * v);
      return Number((Math.max(temp, humidex - windRelief)).toFixed(1));
    }
    return Number(temp.toFixed(1));
  }

  // 3. STREFA KOMFORTU / PRZEJŚCIOWA (10°C < T < 24°C) - Standard IMGW / Jagti
  // Przy łagodnym i umiarkowanym wietrze odczuwalna jest tożsama z temperaturą rzeczywistą.
  // Jedynie silny wiatr (> 20 km/h) daje zauważalne uczucie przewiewu.
  let diff = 0;
  if (ws > 20) {
    diff -= Math.min(1.5, (ws - 20) * 0.05);
  }
  if (temp < 15 && rh > 85 && ws > 12) {
    diff -= 0.3; // wilgotny chłód
  }

  return Number((temp + diff).toFixed(1));
}
